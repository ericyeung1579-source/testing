...
/**
 * On-Chain Performance Test - 25 Assets
 */
...
async function test25Assets() {
  ...
  const receipt1 = await publicClient.waitForTransactionReceipt({ hash1 });
  ...
  const receipt2 = await publicClient.waitForTransactionReceipt({ hash2 });
  const txTime2 = performance.now() - txStart2;

  const gasUsed2 = Number(receipt2.gasUsed);
  const gasPerProof2 = Math.round(gasUsed2 / proofDataArray.length);
